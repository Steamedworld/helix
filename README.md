...
| `TRUSTED_HOME_PLAYBACK_REFRESH_SECRET` | (derived) | HMAC-SHA256 key for signing playback refresh tokens. If absent, derived from `MEDIA_TOKEN_SECRET` via `HMAC(secret, "playback_refresh")` (domain-separated). Set an explicit value in production for auditability. Required in production if neither this nor `MEDIA_TOKEN_SECRET` is set. |
| `TRUSTED_HOME_PLAYBACK_REFRESH_TOKEN_TTL_MS` | `600000` (10 min) | Lifetime of each signed playback refresh token embedded in `PlaybackSource.refreshUrl`. Clients receive a fresh token on every refresh call. |
...
- **Signed refresh tokens** — `PlaybackSource.refreshUrl` contains a short-lived HMAC-SHA256 signed token (`?rt=...`, 10 minutes by default, configurable via `TRUSTED_HOME_PLAYBACK_REFRESH_TOKEN_TTL_MS`). The token is scoped to the user, session, node, and media item — it cannot be reused after expiry or used for a different node/media combination. The signing secret never appears in any browser response. Each refresh call returns a freshly rotated token (new nonce, new `iat`/`exp`). The frontend treats `refreshUrl` as opaque and must not parse or modify the `?rt` parameter.
